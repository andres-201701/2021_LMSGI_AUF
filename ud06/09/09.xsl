...
<!--
    Document   : 09.xsl
    Created on : 20 de mayo de 2021, 9:07
    Author     : daw1
    Description:
...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:template match="/">
        <html>
            <head>
                ...
                <xsl:with-param name="descripcion" select="'Informacion de universidad'"/>
                </xsl:call-template>
            </head>
            <body>
    ...
    <xsl:template match="carrera">
        <div class="carrera">
            <h2><xsl:value-of select="concat(@id,'- ',nombre,' (',plan,')')"/></h2>
            <div>
    ...
    <xsl:template match="asignatura">
        <li><xsl:value-of select="concat('(',@id,')',nombre)"/>
    ...
    <xsl:template match="alumno">
        ...
        <abbr>
            <xsl:attribute name="title">
                <xsl:value-of select="//carrera[@id=current()]/nombre"/>
            </xsl:attribute>
